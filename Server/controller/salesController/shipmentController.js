...
const Shipment = require("../../models/saleModel/shipmentModel")
...
const createProduct = catchAsyncError(async(req, res, next) => {
    try {
        ...
        const manageProduct = await Shipment.create(req.body);
...
const getAllProduct = catchAsyncError(async(req, res, next) => {
    try {
        const productCount = await Shipment.countDocuments();

        //   const apiFeature = new ApiFeatures(Product.find(), req.query)

        const apiFeature = new ApiFeatures(Shipment.find(), req.query)
            .searchByProductName()
...
const getProductDetail = catchAsyncError(async(req, res, next) => {
    try {
        const product = await Shipment.findById(req.params.id);
...
const updateProduct = catchAsyncError(async(req, res, next) => {
    ...
    try {
        let product = await Shipment.findById(req.params.id);

        if (!product) {
            return next(new ErrorHandler("Product not found", 404));
        }

        product = await Shipment.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
...
const deleteMultipleProducts = catchAsyncError(async(req, res, next) => {
    try {
        ...
        await Shipment.deleteMany({ _id: { $in: objectIds } });
