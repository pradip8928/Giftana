const Message = require("../../models/systemModel/messageModel");
...
const createProduct = catchAsyncError(async(req, res, next) => {
    try {
        ...
        const product = await Message.create(req.body);
...
const getAllProduct = catchAsyncError(async(req, res, next) => {
    try {
        ...
        const apiFeature = await new ApiFeatures(Message.find(), req.query)
            .searchByProductName()
...
const updateProduct = catchAsyncError(async(req, res, next) => {
    ...
    try {
        let product = await Message.findById(req.params.id);

        if (!product) {
            return next(new ErrorHandler("Product not found", 404));
        }

        product = await Message.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
...
const deleteMultipleProducts = catchAsyncError(async(req, res, next) => {
    try {
        ...
        await Message.deleteMany({ _id: { $in: objectIds } });
