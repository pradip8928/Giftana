const Discount = require("../../models/promotionModel/discountModel");
...
const createProduct = catchAsyncError(async(req, res, next) => {
    try {
        ...
        const product = await Discount.create(req.body);
...
const getAllProduct = catchAsyncError(async(req, res, next) => {
    try {
        const productCount = await Discount.countDocuments();
        ...
        const apiFeature = await new ApiFeatures(Discount.find(), req.query)
            .searchByProductName()
...
const updateProduct = catchAsyncError(async(req, res, next) => {
    ...
    try {
        let product = await Discount.findById(req.params.id);

        if (!product) {
            return next(new ErrorHandler("Product not found", 404));
        }

        product = await Discount.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
...
const deleteMultipleProducts = catchAsyncError(async(req, res, next) => {
    try {
        ...
        await Discount.deleteMany({ _id: { $in: objectIds } });
