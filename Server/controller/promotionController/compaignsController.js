const Compaign = require("../../models/promotionModel/compaignsModel");
...
const createProduct = catchAsyncError(async(req, res, next) => {
    try {
        ...
        const product = await Compaign.create(req.body);
...
const getAllProduct = catchAsyncError(async(req, res, next) => {
    try {
        const productCount = await Compaign.countDocuments();
        ...
        const apiFeature = await new ApiFeatures(Compaign.find(), req.query)
            .searchByProductName()
...
const getProductDetail = catchAsyncError(async(req, res, next) => {
    try {
        const product = await Compaign.findById(req.params.id);
...
const updateProduct = catchAsyncError(async(req, res, next) => {
    ...
    try {
        let product = await Compaign.findById(req.params.id);

        if (!product) {
            return next(new ErrorHandler("Product not found", 404));
        }

        product = await Compaign.findByIdAndUpdate(req.params.id, req.body, {
            new: true,
            runValidators: true,
...
const deleteMultipleProducts = catchAsyncError(async(req, res, next) => {
    try {
        ...
        await Compaign.deleteMany({ _id: { $in: objectIds } });
